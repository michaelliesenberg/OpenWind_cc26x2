...
void TrueWind_measNotify(float Sample)
{
  ...
  if (WindMeas.pValue != NULL)
  {
    ...
    *p++ =  (uint8_t)mov_calib_status;
    ...
    // Send notification.
    bStatus_t ret = Wind_MeasNotify(gapConnHandle, &WindMeas);

    //WindInd.pValue = WindMeas.pValue;
   // WindInd.len = WindMeas.len;

    //bStatus_t ret = Wind_MeasIndication(gapConnHandle, &WindInd);
